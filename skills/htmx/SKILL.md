...
- `references/server-patterns.md` -- fragment endpoints, validation responses, OOB updates, HX headers, template organization

## Assets

- `assets/search.html` -- simple active-search surface using stable targets
- `assets/form.jinja` -- server-rendered form fragment with validation and request-state feedback

## What Stays Here

Keep focused on mental model and defaults.

- keep here: hypermedia-first rules, fragment boundaries, validation stance, review cues
...
- respond with HTML fragments for htmx surfaces; do not force JSON where HTML is the real contract
...
- use `422` for validation fragments when form well-formed but invalid
- use OOB updates only when one action truly needs to refresh multiple surfaces
- keep Alpine for local UI state, htmx for server round-trips; do not make the page fight itself
...
- do not mix complex client-side state management into a flow that should stay hypermedia-driven
...
- check form validation returns useful fragment updates
- check history behavior intentional
- check auth, CSRF, and escaping handled safely
- check htmx solving the interaction instead of recreating a client framework piecemeal